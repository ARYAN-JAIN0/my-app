...
export async function generateOutboundDraft(lead: Lead) {
  ...
  const result = await runJsonTask<{ subject: string; body: string; signalBreakdown?: Record<string, number>; confidenceScore?: number }>([
    {
      role: "system",
      content:
...
export async function analyzeReply(text: string) {
  ...
  return runJsonTask<ReplyPayload>([
    {
      role: "system",
      content:
...
export async function generateReplyDraft(params: {
  ...
  const context = await retrieveContext(
    `${params.leadName} ${params.company} ${params.intent}`,
    [
  ...
  const result = await runAiTask([
    {
      role: "system",
      content:
        "Write a professional outbound reply draft to the inbound message. Keep it concise and actionable.",
    },
...
export async function buildLeadScoreExplanation(lead: Lead) {
  const result = await runAiTask([
    {
      role: "system",
      content: "Return a short explanation for lead score and outreach strategy.",
    },
    {
      role: "user",
